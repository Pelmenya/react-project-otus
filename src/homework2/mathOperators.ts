...
export const qrt: ScalarOperationTypeOneParametr = (first: number): number => first * first;

export const factorial: ScalarOperationTypeOneParametr = (first: number): number => {
  if (first > 1) {
    return first * factorial(first - 1);
  } else return 1;
};

export const pow: ScalarOperationType = (first: number, second: number): number => {
...
export const mathOperatorsOneParametr: { [key: string]: ScalarOperationTypeOneParametr } = {
  '**': qrt,
  '!': factorial,
};
